...
const alimentationType =  route.params.type

const { dinosaurs, loading, error, fetchDinosaursByAlimentation } = useDinosaurs()

onMounted(async () => {
  await fetchDinosaursByAlimentation(alimentationType)
})

console.log(dinosaurs.value.length)
</script>
<template>
  <div class="category-details-page">
    <h1>🦕 Dinosaures de cette alimentation</h1>
    <p class="subtitle">Découvrez tous les dinosaures appartenant à cette classification</p>

    <p v-if="loading">Chargement...</p>
    <p v-else-if="error" class="error">{{ error }}</p>
    <p v-else-if="dinosaurs?.length === 0" class="empty">
